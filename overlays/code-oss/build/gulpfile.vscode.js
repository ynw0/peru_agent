...
const commit = getVersion(root) || product.commit;
...
function patchWin32DependenciesTask(destinationFolderName) {
	...
	return async () => {
		...
		await Promise.all(deps.map(async dep => {
			const dependency = path.join(cwd, dep);
			const header = await fs.promises.readFile(dependency);
			if (header[0] !== 0x4D || header[1] !== 0x5A) {
				return;
			}
			const basename = path.basename(dep);

			await rcedit(dependency, {
...
const buildRoot = process.env.INDEPENDENT_AI_IDE_BUILD_ROOT || path.dirname(root);
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		if (platform === 'win32' && process.env.INDEPENDENT_AI_IDE_LOCAL_PACKAGE !== '1') {
